...
public class Job {
    public static void main(String[] args) {
        try {
            ...
            SingleOutputStreamOperator<TargetBean> webEvent = TransferHolder.processWeb(webMappedStream);
            ...
            SingleOutputStreamOperator<TargetBean> webPrefectureEvent = TransferHolder.processWebPrefecture(webPrefectureStream);

            //huya-taf相关日志
            SingleOutputStreamOperator<TafSourceEvent> huyaMappedStream = SourceStreamHolder.getTafSourceEventStream(env, huyaKafkaConsumer);
            SingleOutputStreamOperator<TargetBean> huyaEvent = TransferHolder.processTaf(huyaMappedStream, CorpType.HUYA, BgType.HUYA_TAF);

            //yome-taf相关日志
            SingleOutputStreamOperator<TafSourceEvent> yomeMappedStream = SourceStreamHolder.getTafSourceEventStream(env, yomeKafkaConsumer);
            SingleOutputStreamOperator<TargetBean> yomeEvent = TransferHolder.processTaf(yomeMappedStream, CorpType.YOME, BgType.YOME_TAF);
            ...
            SingleOutputStreamOperator<TargetBean> nimoEvent = TransferHolder.processTaf(nimoMappedStream, CorpType.NIMO, BgType.NIMO_TAF);
